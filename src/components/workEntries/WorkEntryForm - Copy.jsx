...
export default function WorkEntryForm({
  ...
  const loadExistingData = async () => {
    try {
      ...
      // Load contract
      const contractResult = await contractService.getContract(initialData.contract_id);
      if (contractResult.success) {
        setSelectedContract(contractResult.data);
        
        // Load template
        if (contractResult.data.template_id) {
          await loadTemplate(contractResult.data.template_id);
        }
      }
    } catch (err) {
