    ...
    for (const sectionLayout of template.pdf_layout.sections) {
      ...
      switch (layout) {
        ...
        case 'signature_box': {
          ...
          const sigAttachment = attachments.find(a =>
        ...
        case 'photo_grid': {
          ...
          yPos = await renderPhotoGrid(pdf, sectionPhotos, sectionLayout, yPos);
          break;
        }
    ...
    if (isFieldTypeSelected('signature')) {
      ...
      if (signatures.length > 0 && !entry.template?.pdf_layout?.sections?.some(s => s.layout === 'signature_box')) {
        for (const sig of signatures) {
          ...
          pdf.setFontSize(10);
          pdf.setFont('helvetica', 'bold');
          pdf.text('Signature', marginLeft, yPos);
          yPos += 5;

          const { embedSignature } = await import('./pdfHelpers');
          await embedSignature(pdf, sig.storage_url, marginLeft, yPos, 60, 30);
          yPos += 38;
        }
      }
    }
