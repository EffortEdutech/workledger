...
class RenderEngineCore {
   ...
   * V2: Full template_section binding support with field labels
   */
  processSection(section, workEntry, legacyBindingRules = null) {
    ...
    if (section.binding_rules) {
      ...
      // KEY FEATURE: Add proper field labels from template
      // When template_section is set, get human-readable labels
      if (section.binding_rules.template_section && workEntry.template) {
        const labels = this.extractFieldLabels(
          section.binding_rules,
          workEntry.template
        );
        if (Object.keys(labels).length > 0) {
          content._labels = labels;
          console.log(`  🏷️ Added ${Object.keys(labels).length} field labels for "${section.section_id}"`);
        }
      }
      
    } else if (blockType === 'photo_grid') {
      // Photo grid: check for filter in binding_rules
      const filter = section.binding_rules?.filter_by_field;
      const photos = extractPhotos(workEntry, filter);
      content = { photos };
      console.log(`  📸 ${photos.length} photos for "${section.section_id}"${filter ? ` (filter: ${filter})` : ''}`);
      
    } else if (blockType === 'signature_box') {
      // Signature box: extract all signatures
      const signatures = extractSignatures(workEntry);
      content = { signatures };
      console.log(`  ✍️ ${signatures.length} signatures for "${section.section_id}"`);
    ...
    // Create render block - include content.title from section.options OR section.content
    const block = {
      ...
      // Title priority: options.title > content.title > section_id
      title: section.options?.title || section.content?.title || null
    };
  ...
  /**
   * Extract field labels from template schema
   * Returns { field_id: "Human Readable Label" } map
   * Used to show proper column headers in reports
   */
  extractFieldLabels(bindingRules, template) {
    ...
    const section = template.fields_schema.sections.find(
      s => s.section_id === template_section
    );
    
    if (!section) {
      console.warn(`  ⚠️ Template section "${template_section}" not found`);
      return labels;
    }
    
    // Extract labels for specific fields, or all fields in section
    for (const field of (section.fields || [])) {
      if (!fields || fields.includes(field.field_id)) {
        labels[field.field_id] = field.field_name;
      }
    }
